...
```{r round-summary-2, fig.width=11, fig.height=11, include = F}
ggplot() + 
...
```{r trait-doy-feature, fig.width = 14, fig.height = 7, include = F}
...
```{r predictors-and-plots, fig.width=12, fig.height=5, include = F}
...
```{r acc-duration-plot, fig.width=7, fig.height=4, include = F}
...
```{r sample-ctmax-summary, fig.width=14, fig.height=5}
...
dlnm_data = full_data %>%  
  ...
  select(collection_date, sp_name, ctmax) %>% 
  group_by(collection_date, sp_name) %>%  
  summarise(mean_ctmax = mean(ctmax, na.rm = T),
            sample = n())
```

```{r general-relationship} 
dlnm_data %>% 
  left_join(lag_temps, by = join_by("collection_date" == "date")) %>% 
...
dates = dlnm_data$collection_date # For each of these dates, make a vector of the past 30 days (including the day of collection). NOTE: Don't use 'unique' dates here since some collections had multiple species
...
for(d in dates){
  ...
  history = lag_temps %>% 
    filter(date <= d & date > d - 61) %>% 
    arrange(desc(date)) %>% 
    mutate(lag = row_number() - 1) %>% 
    select(lag, mean_temp)
  
  temp_vec = scale(history$mean_temp)[,1]
  names(temp_vec) = history$lag
  
  exp_hist = bind_rows(exp_hist, temp_vec)
...
cb_temps = crossbasis(exp_hist, lag = c(0,60), 
...
#default plots
plot(pred_gam_Zs)
plot(pred_gam_Zs, lag=0)
plot(pred_gam_Zs, var = -2)
...
#     
#     min_m = min(y,na.rm = TRUE)
#     max_m = max(y,na.rm = TRUE)
#     m_int = (max_m - min_m) / 60 #61 entries per lag
#     
#     pred2d$exp <- rep(seq(min_m, max_m, m_int), ncol(y)) 
...
# plot_heat(pred_gam_Zs,exp_hist)
