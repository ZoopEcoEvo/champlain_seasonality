...
ggarrange(sample_dates_plot, sp_ctmax_temp, 
          nrow = 1, 
          labels = "AUTO")
```
...
hind1_data %>% 
  ...
  geom_smooth(data = filter(hind1_data, species %in% c("Senecella calanoides", "Limnocalanus macrurus", "Leptodiaptomus sicilis")), se = F) + 
  coord_cartesian(xlim = c(120,300), ylim = c(-6, 5.1)) + 
  labs(x = "Day of Year", 
...
hind2_data %>%  
  ...
  geom_point(alpha = 0.5) +
  geom_smooth() + 
  coord_cartesian(xlim = c(120,300), ylim = c(0, 5.1)) + 
  labs(x = "Day of Year", 
       y = "Predicted Warming Tolerance \n(°C Above Daily Max)") + 
  theme_matt() + 
  theme(legend.position = "right")
