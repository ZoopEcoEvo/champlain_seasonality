...
ctmax_temp.model = lm(data = model_data, ctmax ~ collection_temp * sp_name)
...
full.model = lme4::lmer(data = model_data,
           ctmax ~ temp_cent + size_cent + days_in_lab + 
             (1 + temp_cent + size_cent|sp_name:sex))
...
ggplot(model_coefs, aes(x = sex, y = intercept, colour = species)) + 
  facet_wrap(species~.) + 
  geom_point(size = 3) + 
  scale_colour_manual(values = species_cols) + 
  theme_matt_facets() + 
...
```{r arr-comp-plot, fig.width=8, fig.height=10}
